			...
			try:
				...
				for word in words:
					if word.lower() in exceptions:
						ignore = True
				if ignore == False:
					link = ''.join(['https://rabota.ua',app.find(href=re.compile('apply#apply$'))['href']])
...
class Apply:
	...
	def apply(self,url,cover):
		...
		cover_letter = self.driver.find_element_by_name("ctl00$content$vcVwPopup$vacancyApplyForm$txAreaGreeting")
		cover_letter.send_keys(cover)
		try:
			lang =self.driver.find_element_by_name("ctl00$content$vcVwPopup$vacancyApplyForm$applyControlQuestion$rptLanguages$ctl00$ddlSkills")
			lang.click()
			lang = Select(lang)
			lang.select_by_index(8)
		except:
			pass
		submit = self.driver.find_element_by_id("ctl00_content_vcVwPopup_vacancyApplyForm_hpLnkSendResumeToEmployer")
		submit.click()
		self.driver.implicitly_wait(30)
	def Quit(self):
		pass
